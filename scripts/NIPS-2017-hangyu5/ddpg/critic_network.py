...
class CriticNetwork:
    ...
    def create_q_network(self,state_dim,action_dim):
        ...
        layer1 = tf.nn.elu(tf.matmul(state_input,W1) + b1)
        layer2 = tf.nn.elu(tf.matmul(layer1,W2) + tf.matmul(action_input,W2_action) + b2)
    ...
    def create_target_q_network(self,state_dim,action_dim,net):
        ...
        layer1 = tf.nn.elu(tf.matmul(state_input,target_net[0]) + target_net[1])
        layer2 = tf.nn.elu(tf.matmul(layer1,target_net[2]) + tf.matmul(action_input,target_net[3]) + target_net[4])
