...
def noisy_dense(x, size, name, bias=True, activation_fn=tf.identity, factorized=False):
    ...
    if name == 'worker_0' or name == 'global':
        if size == 18: # check condition action?
            p = sample_noise([128, 18]) # 128 is rnn_size
            global_p_a = p
            q = sample_noise([1, 18]) # 18 is action size
            global_q_a = q
        else:
            p = sample_noise([128, 1]) # 128 is rnn_size
            global_p_v = p
...
class AC_Network():
    def __init__(self,s_size,a_size,scope,trainer,noisy):
        with tf.variable_scope(scope):
            ...
            self.inputs2seq = tf.reshape(self.inputs,shape=[-1,41,1])
	    self.conv1 = tf.nn.elu(tf.nn.conv1d(self.inputs2seq,tf.truncated_normal(shape=[3,1,1],stddev=0.15),stride=1,padding='VALID'))
	    self.conv2 = tf.nn.elu(tf.nn.conv1d(self.inputs2seq,tf.truncated_normal(shape=[5,1,1],stddev=0.15),stride=1,padding='VALID'))
	    self.conv3 = tf.nn.elu(tf.nn.conv1d(self.inputs2seq,tf.truncated_normal(shape=[1,1,1],stddev=0.15),stride=1,padding='VALID'))
	    self.conv4 = tf.nn.elu(tf.layers.max_pooling1d(self.inputs2seq,3,1,'valid'))

            hidden = slim.fully_connected(slim.flatten(tf.concat([self.conv1,self.conv2,self.conv3,self.conv4],axis=1)),128,activation_fn=tf.nn.elu)
            ...
            step_size = tf.shape(self.inputs2seq)[:1]
            ...
            if noisy:
                ...
                self.policy = tf.clip_by_value(tf.add(noisy_dense(rnn_out,name=scope, size=a_size, activation_fn=tf.nn.elu),0.1),0.0,1.0)
		self.policy2prob = tf.add(tf.divide(self.policy, tf.reduce_sum(self.policy)),1e-4)
                self.value = noisy_dense(rnn_out,name=scope, size=1) # default activation_fn=tf.identity
            else:
                #Output layers for policy and value estimations
                self.policy = tf.clip_by_value(tf.add(slim.fully_connected(rnn_out,a_size,
                    activation_fn=tf.nn.elu,
                    weights_initializer=normalized_columns_initializer(0.8),
                    biases_initializer=None),0.1),0.0,1.0)
		self.policy2prob = tf.add(tf.divide(self.policy, tf.reduce_sum(self.policy)),1e-4)
            ...
            if scope != 'global':
                ...
                self.responsible_outputs = tf.reduce_sum(self.policy2prob * self.actions, [1])
                ...
                if noisy:
                    ...
                    self.entropy = - tf.reduce_sum(self.policy2prob * tf.log(self.policy2prob))
